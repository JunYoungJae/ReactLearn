...
Master.prototype.insertMaster = async (req, res) => {    
    try {         
       ...
       var masterNo = await pg.getconnection().query("select nextval('sq_master_no')");
       masterNo = String(masterNo['rows'][0]['nextval']);
       masterNo =  String(_no + masterNo.toString()).substr( String(_no + masterNo.toString()).length-10, 10)
