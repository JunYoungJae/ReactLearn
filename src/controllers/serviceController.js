...
const addService = asyncHandler(async (req, res, next) => {
    ...
    try {
        var item = req.body;
        var serviceNo = await pg.getconnection().query("select nextval('sq_service_no') as service_no");
        serviceNo = String(serviceNo['rows'][0]['service_no']);

        item['serviceNo'] = getTodayYYMMDD() + getSequenceNumber(serviceNo, 8);

        var query = mybatisMapper.getStatement("service", "addService", item, format);   
